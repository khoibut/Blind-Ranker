...
function CreateEntry({ number, value, onChange, onRemove, image, onImageChange, onImageRemove }: CreateEntryProps) {
    ...
    return (
        <div className="border-gray-500 border-1 box-border h-20 rounded-2xl gap-10 flex items-center">
            ...
            }} style={{ backgroundImage: image ? `url(${URL.createObjectURL(image)})` : 'none' }} className="w-25 h-full bg-gray-200 rounded-2xl flex justify-center items-center text-xl select-none hover:cursor-pointer bg-contain bg-center bg-no-repeat">
                {image ? "" : "Add image"}
            </div>
